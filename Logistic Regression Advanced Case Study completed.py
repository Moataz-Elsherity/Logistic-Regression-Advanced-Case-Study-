...
#i belive that it is in fact a problem
#we need to do corss validation anyway because it gives us a fast way to test parameters that might give us better
#results but also because it gives validation to our model and gives us confidence inervals 
#also its an easy way to double check how the model would act under differen params so no excuse to not try it anyway
#it might not work all the time but when it does its worth it. 
#it also gives us better insights on important concerns like overfitting 
#also it allows us to train the model without being biased by adjusting the model on the test data which might 
#make the model biased and inaccurate and no longer tells us how it actually acts with unseen data 
...
clf = LogisticRegression(solver = 'liblinear', max_iter = 500, C=best_score)
...
#hmm it didnt give the same value! not sure why
#it preforms well on the test set
...
#THE LINK IS NOT WORKING SHOWS PAGE NOT FOUND
...
y_pred_test = best_model.predict(Xtestlrstrat)

test_accuracy = accuracy_score(ytestlrstrat, y_pred_test)
